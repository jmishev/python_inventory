...
class TestCase(unittest.TestCase):
    ...
    def test_select_dest_file(self):  # the argument is for testing purposes, avoiding dialog in test mode.
        ...
        file = "C:\\Users\jmishev\Desktop\inventory\REVIZIQ"
        inv.root.title(file)
        self.assertEqual(inv.root.title(), "C:\\Users\jmishev\Desktop\inventory\REVIZIQ")
        # if Canceled nothing happens

    def test_close_with_x(self):
        with patch("inv.write_or_add"):
            inv.good_list = MagicMock()
            inv.good_list = []
            inv.entries[1, 1].get = MagicMock(return_value=None)
            inv.entries[2, 1].get = MagicMock(return_value=None)
            with self.assertRaises(SystemExit):
                inv.close_with_x()
            inv.check_on_screen = MagicMock(return_value="Good to be entered")
            inv.entries[1, 1].get = MagicMock(return_value="1")
            inv.entries[2, 1].get = MagicMock(return_value="1")
    ...
    def test_write_or_add(self, mk1, mk2):
            inv.root.title = MagicMock(return_value="Програма за ревизия")
            self.assertEqual(inv.write_or_add(), "written to excel")
            inv.root.title=MagicMock(return_value="25.xlsX")
    ...
    def test_close_with_button(self, m1):  # event is given by pushing the button
        ...
        with patch("inv.exit"):
            ...
            inv.check_on_screen = MagicMock(return_value="Saving onscreen rejected")
    ...
    def test_check_on_screen(self, m):
        ...
        inv.entries[1, 1].get = MagicMock(return_value=None)
        inv.entries[2, 1].get = MagicMock(return_value="2")
        self.assertEqual(inv.check_on_screen(), "No data")

        inv.entries[1, 1].get = MagicMock(return_value="3,44")
        inv.entries[2, 1].get = MagicMock(return_value="2")
    ...
    def test_check_if_entries_are_numbers(self):
        inv.entries[1, 1].get = MagicMock(return_value="2")
        inv.entries[2, 1].get = MagicMock(return_value="2")
        self.assertTrue(inv.check_if_entries_are_numbers())
        inv.entries[1, 1].get = MagicMock(return_value="m")
        inv.messagebox.showinfo = MagicMock(return_value=True)

    def test_highlight_wrong(self):
        with patch("inv.tk.Entry.get"):
            inv.entries[0, 1].get = MagicMock(return_value="m")
            inv.entries[1, 1].get = MagicMock(return_value="m")
            inv.entries[2, 1].get = MagicMock(return_value="1")
            inv.highlight_wrong()
    ...
    def test_on_return(self, m):
        with patch("inv.root.focus_get"):
            inv.root.focus_get = MagicMock(return_value=inv.entries[0, 1])
            self.assertEqual(inv.on_return("Return"), inv.entries[1, 1])

            inv.root.focus_get = MagicMock(return_value=inv.entries[1, 1])
            self.assertEqual(inv.on_return("Return"), inv.entries[2, 1])

            inv.root.focus_get = MagicMock(return_value=inv.entries[2, 1])
            self.assertEqual(inv.on_return("Return"), inv.entries[0, 1])

    # @patch("inv.check_if_entries_are_numbers")
    @patch("inv.create_good")
    @patch("inv.write_or_add")
    def test_enter_good(self, m, m1):
        # inv.messagebox.showinfo = MagicMock(return_value=True)
        inv.entries[1, 1].get = MagicMock(return_value="3")
        inv.entries[2, 1].get = MagicMock(return_value="1")
        # inv.check_if_entries_are_numbers = MagicMock(return_value=False)
    ...
    def test_set_focus_on_grid_slave(self):
        self.assertEqual(inv.set_focus_on_grid_slave(1, 1), inv.entries[1, 1])
        self.assertEqual(inv.set_focus_on_grid_slave(2, 1), inv.entries[2, 1])

    def test_arrow_moves(self):
        inv.event = MagicMock(keysym="Down", widget=inv.entries[1, 1])
        self.assertEqual(inv.arrow_moves(inv.event), (2, 1))

        inv.event = MagicMock(keysym="Up", widget=inv.entries[2, 1])
        self.assertEqual(inv.arrow_moves(inv.event), (1, 1))

        inv.event = MagicMock(keysym="Right", widget=inv.entries[1, 1])
        self.assertEqual(inv.arrow_moves(inv.event), (1, 2))

        inv.event = MagicMock(keysym="Up", widget=inv.entries[0, 1])
        self.assertEqual(inv.arrow_moves(inv.event), (0, 1))
    ...
    def test_create_good(self):
        ...
        inv.entries[0, 1].get = MagicMock(return_value='a')
    ...
    def test_add_quantity(self):
        inv.entries[0, 1].get = MagicMock(return_value='a')
        inv.entries[1, 1].get = MagicMock(return_value="6")
        inv.entries[2, 1].get = MagicMock(return_value="5")
        inv.v.get = MagicMock(return_value=1)
    ...
    def test_calculate_totals(self):
        ...
        good1 = inv.Goods("No name", 6, 5)
        inv.good_list = [good, good1]
        self.assertEqual(inv.calculate_total(), 48)

    # def test_wrtie_to_database(self):
    #     good = inv.Goods("opa", 3, 4)
    #     inv.good_list = [good]
